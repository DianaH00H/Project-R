---
title: "Redes_multinomiales"
author: "Diana"
format: 
    html:
      toc: true
      html-math-method: katex
      embed-resources: true
      self-contained-math: true
      df-print: kable
editor: source
---

## Ejemplo: Transporte

```{r}
#| eval: false
install.packages("bnlearn")
```

```{r}
library(bnlearn)
```

```{r}
data = read.csv("C:/Users/diana/Documents/Project-R/data/survey.csv", stringsAsFactors = TRUE)
head(data)
```

```{r}
dag = empty.graph(nodes = c('A', 'S', 'E', 'O', 'R', 'T'))
```

```{r}
arc_set = matrix(c('A', 'E',
                   'S', 'E',
                   'E', 'O',
                   'E', 'R',
                   'O', 'T',
                   'R', 'T'), byrow = TRUE, ncol = 2,
                 dimnames = list(NULL, c('from', 'to')))
```

```{r}
arcs(dag) = arc_set
```

```{r}
dag
```


```{r}
bn_mle = bn.fit(dag, data=data, method = 'mle')
```

```{r}
bn_mle
```
	
$H_0: T {\perp \!\!\! \perp} E|O,R$
$H_a: T {\not\!\perp \!\!\! \perp} E|O,R$

1- Prueba $G²$: Cociente de verosimilitudes
2- $x²$ de Pearson: Prueba de bondad de ajuste

Tienen una ditribución $x²$


```{r}
ci.test('T', 'E', c('O', 'R'), test = 'mi', data = data)
```

```{r}
ci.test('T', 'E', c('O', 'R'), test = 'x2', data = data)
```



$H_{0}: T \perp \!\!\!\perp O \mid R \quad \quad \quad H_{a}: T \not\!\perp\!\!\!\perp O \mid R$


```{r}
ci.test('T', 'O', 'R', test = 'x2', data = data)
```



```{r}
arc.strength(dag, data= data, criterion = 'mi')
```

```{r}
score(dag, data = data, type = 'bic')
```

```{r}
score(dag, data = data, type = 'aic')
```

```{r}
best_dag = hc(data)
```

```{r}
modelstring(best_dag)
```

```{r}
score(best_dag, data = data, type = "bic")
score(best_dag, data = data, type = "aic")
```

```{r}
install.packages("BiocManager")
BiocManager::install("Rgraphviz")
```

```{r}
graphviz.plot(best_dag, shape = "ellipse")
```

```{r}
bn = bn.fit(best_dag, data = data)
```

```{r}
cpquery(bn, event = (T == "train" & S == "F"), evidence = (E == "uni") , n = 10^6)
```
```{r}
cpquery(bn, event = (T == "train" & S == "F"), evidence = list(E = "uni"), method = "lw")
```

```{r}
cpquery(bn, event = (T == "train" & S == "M"), evidence = ((A == "young") & (E == "uni")) | (A == "adult"), n = 10^6)
```

```{r}

```

